    ...
    public void pvStock () {
        databaseReference.child("product").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.hasChild("K001")) {
                    countStock = snapshot.child("K001").child("stock").getValue(String.class);
                    Stock1.setText("STOCK: "+countStock);
                }

                if (snapshot.hasChild("K002")) {
                    countStock = snapshot.child("K002").child("stock").getValue(String.class);
                    Stock2.setText("STOCK: "+countStock);
                }

                if (snapshot.hasChild("K003")) {
                    countStock = snapshot.child("K003").child("stock").getValue(String.class);
                    Stock3.setText("STOCK: "+countStock);
                }

                if (snapshot.hasChild("K004")) {
                    countStock = snapshot.child("K004").child("stock").getValue(String.class);
